...
export async function uniqueItem(params: any) {
  return request('/api/dict/item/unique', {
    params,
  });
}
...
export async function getItemById(params: { id: string }) {
  return request(`/api/dict/item/${params.id}`);
}
/**
 * 添加类型
 * @param params
...
export async function removeTypeByIds(params: { ids: [] }) {
  return request(`/api/dict/type/${new Array(params.ids).join(',')}`, { method: 'DELETE' });
}
/**
 * 删除字典项
 * @param params
...
export async function removeItemByIds(params: { ids: [] }) {
  return request(`/api/dict/item/${new Array(params.ids).join(',')}`, { method: 'DELETE' });
}
/**
 * 根据ID更新状态
 * @param params
...
export async function updateDefaultById(params: { id: string; isDefault: string }) {
  return request(`/api/dict/item/${params.id}/default/${params.isDefault}`, {
    method: 'PUT',
  });
}
